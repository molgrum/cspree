...
pid=`ps ax | grep -v grep | grep "fteqw.sv -game gmapn +exec ports/port1.cfg" | awk '{print $1}'`
if [ "$pid" != "" ]; then kill -9 $pid; fi;

# Kill 28502
pid=`ps ax | grep -v grep | grep "/bin/sh ./run/port2.sh" | awk '{print $1}'`
if [ "$pid" != "" ]; then kill -9 $pid; fi;
pid=`ps ax | grep -v grep | grep "fteqw.sv -game gmapn +exec ports/port2.cfg" | awk '{print $1}'`
if [ "$pid" != "" ]; then kill -9 $pid; fi;

# Kill 28503
pid=`ps ax | grep -v grep | grep "/bin/sh ./run/port3.sh" | awk '{print $1}'`
if [ "$pid" != "" ]; then kill -9 $pid; fi;
pid=`ps ax | grep -v grep | grep "fteqw.sv -game gmapn +exec ports/port3.cfg" | awk '{print $1}'`
if [ "$pid" != "" ]; then kill -9 $pid; fi;

# Kill 28504
pid=`ps ax | grep -v grep | grep "/bin/sh ./run/port4.sh" | awk '{print $1}'`
if [ "$pid" != "" ]; then kill -9 $pid; fi;
pid=`ps ax | grep -v grep | grep "fteqw.sv -game gmapn +exec ports/port4.cfg" | awk '{print $1}'`
if [ "$pid" != "" ]; then kill -9 $pid; fi;
