...
if ps ax | grep -v grep | grep "fteqw.sv -game gmapn +exec port1.cfg" > /dev/null
then
...
if ps ax | grep -v grep | grep "fteqw.sv -game gmapn +exec port2.cfg" > /dev/null
then
    echo "[ALREADY RUNNING]"
else
...
if ps ax | grep -v grep | grep "fteqw.sv -game gmapn +exec port3.cfg" > /dev/null
then
    echo "[ALREADY RUNNING]"
else
...
if ps ax | grep -v grep | grep "fteqw.sv -game gmapn +exec port4.cfg" > /dev/null
then
    echo "[ALREADY RUNNING]"
else
